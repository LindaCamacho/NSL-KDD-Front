...
BASE_DIR = Path(__file__).resolve().parent.parent  # apunta a la raíz del proyecto

app.mount("/static", StaticFiles(directory=BASE_DIR / "static"), name="static")
templates = Jinja2Templates(directory=BASE_DIR / "templates")


# Diccionario para guardar DataFrame actual
uploaded_dfs = {}
...
async def upload_file(request: Request, file: UploadFile = File(...)):
    ...
    info = {
        ...
        "primeras_filas": df.head().to_html(classes="preview-table", index=False)
    }
...
async def process(request: Request, selected_columns: list[str] = Form(...), limit: int = Form(5)):
    ...
    for col in selected_columns:
        ...
        summary_df = pd.concat([summary_df, pd.DataFrame({
            "Columna": [col],
            "Valores Únicos": [', '.join(map(str, unique_vals)) + more_flag]
        })])
